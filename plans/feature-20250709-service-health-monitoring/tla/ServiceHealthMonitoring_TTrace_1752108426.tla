---- MODULE ServiceHealthMonitoring_TTrace_1752108426 ----
...
_inv ==
    ~(
        ...
        discovery_registry = ({})
        /\
        last_check_time = ((s1 :> 1 @@ s2 :> 0 @@ s3 :> 0))
        /\
        failed_checks = ((s1 :> 0 @@ s2 :> 0 @@ s3 :> 0))
        /\
        health_checks = ({})
        /\
        health_status = ((s1 :> "HEALTHY" @@ s2 :> "UNKNOWN" @@ s3 :> "UNKNOWN"))
    )
----
...
\* to `JsonSerialize`. For example, a sub-sequence of _TETrace.
    \* ASSUME
    \*     LET J == INSTANCE Json
    \*         IN J!JsonSerialize("ServiceHealthMonitoring_TTrace_1752108426.json", _TETrace)
...
\*
\*trace == IODeserialize("ServiceHealthMonitoring_TTrace_1752108426.bin", TRUE)
\*
\*=============================================================================
\*
...
trace == 
    ...
    ([discovery_registry |-> {},last_check_time |-> (s1 :> 0 @@ s2 :> 0 @@ s3 :> 0),failed_checks |-> (s1 :> 0 @@ s2 :> 0 @@ s3 :> 0),health_checks |-> {},health_status |-> (s1 :> "STARTING" @@ s2 :> "UNKNOWN" @@ s3 :> "UNKNOWN")]),
    ([discovery_registry |-> {},last_check_time |-> (s1 :> 1 @@ s2 :> 0 @@ s3 :> 0),failed_checks |-> (s1 :> 0 @@ s2 :> 0 @@ s3 :> 0),health_checks |-> {[service |-> s1, timestamp |-> 1, result |-> FALSE]},health_status |-> (s1 :> "STARTING" @@ s2 :> "UNKNOWN" @@ s3 :> "UNKNOWN")]),
    ([discovery_registry |-> {},last_check_time |-> (s1 :> 1 @@ s2 :> 0 @@ s3 :> 0),failed_checks |-> (s1 :> 0 @@ s2 :> 0 @@ s3 :> 0),health_checks |-> {},health_status |-> (s1 :> "HEALTHY" @@ s2 :> "UNKNOWN" @@ s3 :> "UNKNOWN")])
    >>
----
...
---- CONFIG ServiceHealthMonitoring_TTrace_1752108426 ----
CONSTANTS
    Services = { s1 , s2 , s3 }
    MaxHealthChecks = 5
    s2 = s2
...
ALIAS
    _expression
=============================================================================
\* Generated on Wed Jul 09 21:47:07 BRT 2025